...
Welcome to **Stage 7** of the AI.Nova project tutorial! In this stage, you will explore the comprehensive **authentication and authorization system** built into the project. This production-ready identity system includes JWT-based authentication, role and permission management, session handling, two-factor authentication, and much more.
...
Before diving into the technical architecture, it's important to understand that the AI.Nova project fundamentally supports only **two authentication methods**. All other sign-in options are built on top of these core methods.
...
All alternative authentication methods in the AI.Nova ultimately generate a 6-digit OTP code and perform an **automatic OTP sign-in** behind the scenes:
...
- **Security**: Tokens are signed using a secret key configured in [`appsettings.json`](/src/Server/AI.Nova.Server.Api/appsettings.json)

#### Token Configuration

You can configure token expiration in [`appsettings.json`](/src/Server/AI.Nova.Server.Api/appsettings.json):
...
User sessions are persisted in the database through the [`UserSession`](/src/Server/AI.Nova.Server.Api/Features/Identity/Models/UserSession.cs) entity:
...
From [`IdentityController.cs`](/src/Server/AI.Nova.Server.Api/Features/Identity/IdentityController.cs):
...
External provider settings are configured in [`appsettings.json`](/src/Server/AI.Nova.Server.Api/appsettings.json) under the `Authentication` section:
...
From [`IdentityController.ExternalSignIn.cs`](/src/Server/AI.Nova.Server.Api/Features/Identity/IdentityController.ExternalSignIn.cs):
...
**Location**: [`/src/Server/AI.Nova.Server.Api/appsettings.json`](/src/Server/AI.Nova.Server.Api/appsettings.json)
...
Each token type has a corresponding `RequestedOn` timestamp in the [`User`](/src/Server/AI.Nova.Server.Api/Features/Identity/Models/User.cs) entity:
...
From [`IdentityController.EmailConfirmation.cs`](/src/Server/AI.Nova.Server.Api/Features/Identity/IdentityController.EmailConfirmation.cs):
...
Bit AI.Nova includes built-in support for **Keycloak**, a free, open-source identity and access management solution. Keycloak provides enterprise-grade features like:
...
The Keycloak instance comes pre-configured with the following demo accounts (Provided by [src\Server\AI.Nova.Server.AppHost\Infrastructure\Realms\dev-realm.json](..\src\Server\AI.Nova.Server.AppHost\Infrastructure\Realms\dev-realm.json)):
...
The AI.Nova template integrates Keycloak with ASP.NET Core Identity through a custom mapping system in [`AppUserClaimsPrincipalFactory`](/src/Server/AI.Nova.Server.Api/Features/Identity/Services/AppUserClaimsPrincipalFactory.cs):
...
* [How does a `refresh token` function in a AI.Nova project template?](https://deepwiki.com/search/how-does-a-refresh-token-funct_6a75fa66-ab98-4367-bd1a-24b081fbf88c)
