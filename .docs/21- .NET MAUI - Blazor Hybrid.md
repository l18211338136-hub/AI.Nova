...
4. [AI.Nova.Client.Windows Advantages](#ai.novaclientwindows-advantages) - Better Windows support
...
The `AI.Nova.Client.Maui` and `AI.Nova.Client.Windows` projects demonstrate this architecture in action.
...
**Location**: [`/src/Client/AI.Nova.Client.Maui/Platforms/Android/MainActivity.cs`](/src/Client/AI.Nova.Client.Maui/Platforms/Android/MainActivity.cs)
...
#### iOS - Universal Links
For iOS, this is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `AI.Nova.Client.Web` project. No manual configuration is needed per page!

**Location**: [`/src/Client/AI.Nova.Client.Web/wwwroot/.well-known/apple-app-site-association`](/src/Client/AI.Nova.Client.Web/wwwroot/.well-known/apple-app-site-association)
...
### ApplicationVersion in AI.Nova.Client.Maui

The `ApplicationVersion` property in the `AI.Nova.Client.Maui.csproj` file manages the app's version for store submissions and updates.
...
## AI.Nova.Client.Windows Advantages

The `AI.Nova.Client.Windows` project offers several advantages over the MAUI Windows output:

### Key Benefits

1. **Broader OS Support**: Works on Windows 7, 8, 10, and 11
   - In contrast, `AI.Nova.Client.Maui`'s Windows output only works on Windows 10 and 11
...
The app checks the WebView version on Android to ensure compatibility in [`App.xaml.cs`](/src/Client/AI.Nova.Client.Maui/App.xaml.cs):
...
The MAUI project is located at: `src/Client/AI.Nova.Client.Maui/`
...
The [`MauiProgram.cs`](/src/Client/AI.Nova.Client.Maui/MauiProgram.cs) file is the starting point of the MAUI application:
...
Services specific to MAUI are registered in [`MauiProgram.Services.cs`](/src/Client/AI.Nova.Client.Maui/MauiProgram.Services.cs):
...
The [`MainPage.xaml`](/src/Client/AI.Nova.Client.Maui/MainPage.xaml) file hosts the `BlazorWebView` component, which renders Blazor content inside a native WebView:
...
- It renders the `Routes` component (from `AI.Nova.Client.Core`) inside the `#app-container` element
- All your Blazor components render inside this WebView, just like in the browser!

### 4. BlazorWebView Customization

The `SetupBlazorWebView()` method in [`MauiProgram.cs`](/src/Client/AI.Nova.Client.Maui/MauiProgram.cs) customizes the WebView for each platform:
...
Implements `IStorageService` using MAUI's `Preferences` API in [`MauiStorageService.cs`](/src/Client/AI.Nova.Client.Maui/Infrastructure/Services/MauiStorageService.cs):
...
Implements `IExternalNavigationService` for opening external URLs in [`MauiExternalNavigationService.cs`](/src/Client/AI.Nova.Client.Maui/Infrastructure/Services/MauiExternalNavigationService.cs):
