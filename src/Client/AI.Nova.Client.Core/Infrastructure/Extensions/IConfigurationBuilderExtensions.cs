...
public static partial class IConfigurationBuilderExtensions
{
    ...
    public static IConfigurationBuilder AddClientConfigurations(this IConfigurationBuilder builder, string clientEntryAssemblyName)
    {
        ...
        var sharedAssembly = Assembly.Load("AI.Nova.Shared");

        configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("AI.Nova.Shared.appsettings.json")!);

        var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"AI.Nova.Shared.appsettings.{AppEnvironment.Current}.json");
        if (envSharedAppSettings != null)
        {
            configBuilder.AddJsonStream(envSharedAppSettings);
        }

        var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "AI.Nova.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("AI.Nova.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"AI.Nova.Client.Core.appsettings.{AppEnvironment.Current}.json");
