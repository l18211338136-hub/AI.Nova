...
public partial class AttachmentController : AppControllerBase, IAttachmentController
{
    ...
    {
        var attachments = await DbContext.Attachments.Where(p => p.Id == attachmentId && (kinds == null || (p.Kind.HasValue && kinds.Contains(p.Kind.Value)))).ToArrayAsync(cancellationToken);
